...
function calcularImpacto(distancia, modo) {
    const coeficiente = modo === 'bicicleta' ? 0.2 : 0.3; // Valores fictícios de emissão evitada
...
function calcularImpacto(sacolas) {
    // Valores aproximados (exemplo fictício): 1 sacola plástica equivale a 0,02 kg de CO₂.
    const impactoCO2 = sacolas * 0.02;
    return impactoCO2.toFixed(2); // Retorna o impacto total em kg de CO₂ evitado
}
...
function calcularImpacto(dias) {
    // Valores aproximados (exemplo fictício): 1 dia de compostagem equivale a evitar 0,5 kg de resíduos em aterros.
    const impactoResiduos = dias * 0.5;
    return impactoResiduos.toFixed(2); // Retorna o impacto total em kg de resíduos evitados
}
...
function registrarCompromissos() {
    ...
    // Desmarcar os checkboxes após registrar
